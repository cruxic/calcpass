...
function makeCardRows(words) {
    var rowNum = 1;
    var rows = new Array();
    var footer = '<div class="row odd title">' +
        'Adam B. 2018 (Type 1)' +
        '</div>';
    rows.push(footer);
    var header = '<div class="row even header">' +
        '<div class="num">#</div>' +
...
function makeCards(rows, nPerCard) {
    var cards = new Array();
    var i = 0;
    var n, lines;
    while (i < rows.length) {
...
function onLoad() {
    ...
    var cards = makeCards(makeCardRows(words), 22);
    var elm;
